...
class TestDatumInContext(unittest.TestCase):
    ...
    def test_DatumInContext_in_context(self):
        ...
        assert DatumInContext(3).in_context(path=Fields("foo"), context="whatever").in_context(
            path=Fields("baz"), context="whatever"
        ) == DatumInContext(3).in_context(
            path=Fields("foo"), context=DatumInContext("whatever").in_context(path=Fields("baz"), context="whatever")
        )
...
class TestJsonPath(unittest.TestCase):
    ...
    def test_index_value(self):
        self.check_cases([("[0]", [42], [42]), ("[5]", [42], []), ("[2]", [34, 65, 29, 59], [29]), ("[0]", None, [])])
    ...
    def test_slice_paths(self):
        self.check_paths([("[*]", [1, 2, 3], ["[0]", "[1]", "[2]"]), ("[1:]", [1, 2, 3, 4], ["[1]", "[2]", "[3]"])])
    ...
    def test_slice_auto_id(self):
        jsonpath.auto_id_field = "id"
        self.check_cases(
            [("[*].id", [1, 2, 3], ["[0]", "[1]", "[2]"]), ("[1:].id", [1, 2, 3, 4], ["[1]", "[2]", "[3]"])]
        )
    ...
    def test_update_fields(self):
        self.check_update_cases(
            [({"foo": 1}, "foo", 5, {"foo": 5}), ({"foo": 1, "bar": 2}, "$.*", 3, {"foo": 3, "bar": 3})]
        )
    ...
    def test_update_index(self):
        self.check_update_cases([(["foo", "bar", "baz"], "[0]", "test", ["test", "bar", "baz"])])

    def test_update_slice(self):
        self.check_update_cases([(["foo", "bar", "baz"], "[0:2]", "test", ["test", "test", "baz"])])
